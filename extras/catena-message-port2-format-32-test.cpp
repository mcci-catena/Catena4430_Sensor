...
Name:   catena-message-port2-format-32-test.cpp

Function:
    Generate test vectors for port 0x02 format 0x32 messages.
...
void encodeMeasurement(Buffer &buf, Measurements &m)
    ...
    if (m.Light.fValid)
        ...
        buf.push_back_be(encodeLight(m.Light.v.lux));
        }
...
void logMeasurement(Measurements &m)
    ...
    if (m.Light.fValid)
        {
        std::cout << pad.get() << "Light " << m.Light.v.lux;
        }
